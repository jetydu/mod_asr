            ...
            switch (cjp->type) {
                ...
                case cJSON_Object: {
                    ...
                    if ((pvt = (switch_da_t *) switch_channel_get_private(channel, "asr"))) {
                        ...
                        if ((cjc2 = cJSON_GetObjectItem(cjp, "begin_time")) &&
                            (cjc3 = cJSON_GetObjectItem(cjp, "time"))) {
                            ...
                            if (ses) {
                                ...
                                if (argc > 0) {
                                    switch_snprintf(filename, sizeof(filename), "%s-%s-%d.wav", argv[argc - 1],
                                                    currtime, pvt->datalen);
                                } else {
                                    switch_snprintf(filename, sizeof(filename), "00000000000-%s-%d.wav", currtime,
                                                    pvt->datalen);
                                }

                                switch_snprintf(fullpath, sizeof(fullpath), "%s/%s", SWITCH_GLOBAL_dirs.recordings_dir,
                                                filename);
...
//    char fullpath[200];
    ...
    switch_time_t now = switch_micro_time_now();
    switch_time_exp_lt(&tm, now);
    switch_snprintf(currtime, sizeof(currtime), "%0.4d%0.2d%0.2d%0.2d%0.2d%0.2d%0.6d", tm.tm_year + 1900, tm.tm_mon + 1,
                    tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, tm.tm_usec);
    switch_snprintf(filepath, sizeof(filepath), "../recordings/");
        ...
        if (pvt->request) {
            ...
            if (pvt->request->start() < 0) {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                                  "onTaskFailed --- ASR Start Failed channel:%s\n", switch_channel_get_name(channel));
                if (switch_channel_up(channel)) {
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                                      "onTaskFailed --- Releasing channel : %s\n", switch_channel_get_name(channel));
                    switch_channel_hangup(channel, SWITCH_CAUSE_NORMAL_CLEARING);
                } else {
                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                                      "onTaskFailed --- Channel has been released : %s\n",
                                      switch_channel_get_name(channel));
                }
                ...
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
                                  "onTaskFailed --- ASR Start Succeed channel:%s\n", switch_channel_get_name(channel));
                pvt->cbParam.bSend = true;
            }
        }
        /*  if (pvt->request->start() < 0)
...
//            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "ASR ************SWITCH_ABC_TYPE_READ_REPLACE****************1:%s, channel state: %i, call state: %i\n", switch_channel_get_name(channel), switch_channel_get_state(channel), switch_channel_get_callstate(channel));
            ...
            if (! switch_string_match(pvt->leg, sizeof(pvt->leg), "bleg", 4)) {
                        ...
                        if (pvt->request) {
                            ...
                            if (CheckData(frame_data1, frame_len)) {
                                if (pvt->request->sendAudio(frame_data, frame_len) <= 0) {
                                    switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
                                                      "SendAudio  Failed:%s framedata:%s\n",
                                                      switch_channel_get_name(channel),
                                                      ToHexString(frame_data1, frame_len).c_str());
                                    // pthread_mutex_lock(&(pvt->mutex));
                                    pvt->cbParam.bSend = false;
...
SWITCH_STANDARD_APP(start_asr_session_function)
        ...
        && (argc = switch_separate_string(lbuf, ' ', argv, (sizeof(argv) / sizeof(argv[0])))) >= 6) {
            ...
            if ((status = switch_core_media_bug_add(session, "asr", NULL, asr_callback, pvt, 0,
                                                    SMBF_READ_REPLACE  | SMBF_WRITE_REPLACE | SMBF_NO_PAUSE |
                                                    SMBF_ONE_ONLY, &(pvt->bug))) != SWITCH_STATUS_SUCCESS) {
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_NOTICE,
                              "%s switch_core_media_bug_add OK!\n", switch_channel_get_name(channel));
            ...
            if (pvt->request) {
                ...
                pvt->request->setInverseTextNormalization(true); // 设置是否在后处理中执行ITN, 可选参数. 默认false
                pvt->request->setSemanticSentenceDetection(true); // 设置是否语义断句, 可选参数. 默认false
                pvt->request->setMaxSentenceSilence(
                        pvt->iSilence); //语音断句检测阈值，一句话之后静音长度超过该值，即本句结束，合法参数范围200～2000(ms)，默认值800ms
                pvt->request->setToken(pvt->tokenid); // 设置账号校验token, 必填参数
                if (switch_string_match(pvt->sUrl, sizeof(pvt->sUrl), "null", 4)) {
                    pvt->request->setUrl(pvt->sUrl);
                }
